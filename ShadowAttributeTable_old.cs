...
public class ShadowAttributeTable_v3_WIP : Module {
	...
	public ShadowAttributeTable_v3_WIP(RAMRange r, NameTable nts) {
		...
		PPUAddress = VWord.New(NES.ram, "ShadowAttributeTable_PPUAddress");
		Table	= Array<VByte>.New((U8)(64 * ntCount), r, "ShadowAttributeTable_attributeTable");
		_temp = VByte.New(NES.ram, "ShadowAttributeTable_temp");
	}
	...
	public void Clear(NameTable nts) {
		...
		AL.Comment("Clear attribute table");
...
public class ShadowAttributeTable_v1 : Module {
	...
	public VByte UpdateSingleTile(Func<IOperand> x, Func<IOperand> y, Func<IOperand> color, VByte temp) {
		...
		Stack.Preserve(A, () => {   //preserve attr index, wait to put into X, because X is needed as an index to X() and Y() values
			AL.Comment("temp = attr tile mask index");
			temp.Set(A.Set(x()).Divide(16).And(1));
			temp.Set(z => A.Set(y()).Divide(8).And(0b10).Or(z));
		});
		X.Set(A);	//X = attr index
		A.Set(AL.LabelFor(Palettes)[Y.Set(color())]).And(AL.LabelFor(AttributeBit)[Y.Set(temp)]);
		Stack.Preserve(A, () => {	//preserve tile color
			Table[X].Set(z => z.And(AL.LabelFor(AttributeBitMask)[Y]));	//clear current tile palette
		});
	...
	[DataSection]
	public void AttributeBitMask() => AL.Raw(
		AttributeMaskID.TopLeftMask,
		AttributeMaskID.TopRightMask,
		AttributeMaskID.BottomLeftMask,
		AttributeMaskID.BottomRightMask
	...
	[DataSection]
	public void AttributeBit() => AL.Raw(
		AttributeMaskID.TopLeft,
		AttributeMaskID.TopRight,
		AttributeMaskID.BottomLeft,
		AttributeMaskID.BottomRight
