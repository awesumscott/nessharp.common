...
public static class Hardware {
	...
	public static void LoadPalettes(Action paletteDataSection, bool waitForVBlank = true) {
		AL.Comment("Load palettes");
		if (waitForVBlank)
			WaitForVBlank();
		NES.PPU.SetAddress(NES.MemoryMap.Palette);
		Loop.Repeat_PostInc(X.Set(0), 32, _ => {
			NES.PPU.Data.Write(AL.LabelFor(paletteDataSection)[X]);
		});
	}

	public static void ClearRAM() {
		Loop.Repeat_PostInc(X.Set(0), 256, _ => {
			A.Set(0);
			STA(AL.Addr(0x0000)[X]);
			STA(AL.Addr(0x0100)[X]);
			STA(AL.Addr(0x0300)[X]);
			STA(AL.Addr(0x0400)[X]);
			STA(AL.Addr(0x0500)[X]);
			STA(AL.Addr(0x0600)[X]);
			STA(AL.Addr(0x0700)[X]);
			A.Set(0xFE);
			STA(AL.Addr(0x0200)[X]);
		});
	}
